...
class m160119_070234_wechat_message extends \yii\db\Migration {
	...
	public function up() {
		...
		$this->createTable('{{%wechat_message}}', [
			...
			'msg_type' => Schema::TYPE_STRING . '(50) not null comment "MsgType, 消息类型: text(文本消息), image(图片消息), voice(语音消息), video(视频消息), shortvideo(小视频消息), location(地理位置消息), link(链接消息), link(链接消息), event(事件)"',
